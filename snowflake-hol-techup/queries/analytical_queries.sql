USE database AZURE_DATABRICKS_UC;
USE SCHEMA "gold";
...
SELECT 
    ...
    CONCAT('$', TO_CHAR("total_revenue", 'FM999,999,999,990.00')) as "total_revenue",
    CONCAT('$', TO_CHAR("total_profit", 'FM999,999,999,990.00')) as "total_profit",
    CONCAT(ROUND("avg_margin", 1), '%') as "avg_margin",
    TO_CHAR("total_orders", 'FM999,999,999,990') as "total_orders",
    TO_CHAR("total_customers", 'FM999,999,999,990') as "total_customers",
    TO_CHAR("new_customers", 'FM999,999,999,990') as "new_customers",
    CONCAT(ROUND("new_customers" * 100.0 / NULLIFZERO("total_customers"), 1), '%') as "new_customer_rate"
FROM business_kpis;
...
SELECT 
    ...
    "customer_segment",
    "ltv_tier",
    CONCAT('$', TO_CHAR("customer_lifetime_value", 'FM999,999,999,990.00')) as "lifetime_value",
    "total_orders",
    CONCAT('$', TO_CHAR("average_order_value", 'FM999,999,999,990.00')) as "avg_order_value",
    "favorite_category",
    CASE 
...
SELECT 
    ...
    CONCAT('$', TO_CHAR(AVG("customer_lifetime_value"), 'FM999,999,999,990.00')) as "avg_ltv",
    ROUND(AVG("total_orders"), 1) as "avg_orders",
    CONCAT('$', TO_CHAR(AVG("average_order_value"), 'FM999,999,999,990.00')) as "avg_order_value",
    TO_CHAR(ROUND(AVG("churn_risk_score"), 2)) as "avg_churn_risk"
FROM "customer_metrics"
...
SELECT 
    ...
    "brand",
    "performance_tier",
    CONCAT('$', TO_CHAR("total_revenue", 'FM999,999,999,990.00')) as "revenue",
    TO_CHAR("total_quantity_sold", 'FM999,999,999,990') as "units_sold",
    CONCAT('$', TO_CHAR("current_price", 'FM999,999,999,990.00')) as "price",
...
SELECT
    ...
    CONCAT('$', TO_CHAR(SUM("total_revenue"), 'FM999,999,999,990.00')) as "total_revenue",
    CONCAT('$', TO_CHAR(AVG("total_revenue"), 'FM999,999,999,990.00')) as "avg_revenue_per_product",
...
SELECT
    ...
    COUNT(*) as "product_count",
    CONCAT('$', TO_CHAR(SUM("total_revenue"), 'FM999,999,999,990.00')) as "total_revenue",
    CONCAT('$', TO_CHAR(AVG("total_revenue"), 'FM999,999,999,990.00')) as "avg_revenue",
    ROUND(AVG("average_rating"), 1) as "avg_rating",
...
SELECT 
    ...
    CASE 
        ...
        ELSE 'Normal'
    END as "alert_type",
    CONCAT('$', TO_CHAR("inventory_value", 'FM999,999,999,990.00')) as "inventory_value"
...
SELECT 
    CONCAT("year", '-', LPAD(TO_VARCHAR("month"), 2, '0')) as "month_year",
    CONCAT('$', TO_CHAR(SUM("gross_revenue"), 'FM999,999,999,990.00')) as "revenue",
    TO_CHAR(SUM("total_orders"), 'FM999,999,999,990') as "orders",
    TO_CHAR(SUM("total_customers"), 'FM999,999,999,990') as "customers",
    CONCAT('$', TO_CHAR(AVG("average_order_value"), 'FM999,999,999,990.00')) as "avg_order_value",
    ROUND(AVG("gross_margin_percentage"), 1) as "margin_percent"
FROM "sales_summary"
WHERE "date_key" >= DATEADD(DAY, -365, CURRENT_DATE())
GROUP BY "year", "month"
...
SELECT 
    ...
    CONCAT('$', TO_CHAR(SUM("gross_revenue"), 'FM999,999,999,990.00')) as "total_revenue",
    CONCAT('$', TO_CHAR(SUM("gross_revenue") / NULLIFZERO(COUNT(DISTINCT "date_key")), 'FM999,999,999,990.00')) as "avg_daily_revenue",
    TO_CHAR(SUM("total_orders"), 'FM999,999,999,990') as "total_orders",
    ROUND(SUM("total_orders") / NULLIFZERO(COUNT(DISTINCT "date_key")), 0) as "avg_daily_orders"
FROM "sales_summary"
WHERE "date_key" >= DATEADD(DAY, -365, CURRENT_DATE())
GROUP BY "quarter"
ORDER BY "quarter";
...
SELECT 
    ...
    CONCAT('$', TO_CHAR(AVG("gross_revenue"), 'FM999,999,999,990.00')) as "avg_daily_revenue",
...
SELECT 
    ...
    CONCAT('$', TO_CHAR(SUM("gross_revenue"), 'FM999,999,999,990.00')) as "total_revenue",
    TO_CHAR(SUM("total_orders"), 'FM999,999,999,990') as "total_orders",
    TO_CHAR(SUM("total_customers"), 'FM999,999,999,990') as "total_customers",
    CONCAT('$', TO_CHAR(AVG("average_order_value"), 'FM999,999,999,990.00')) as "avg_order_value",
    ROUND(AVG("gross_margin_percentage"), 1) as "avg_margin"
FROM "sales_summary"
WHERE "date_key" >= DATEADD(DAY, -90, CURRENT_DATE())
...
WITH customer_cohorts AS (
    SELECT 
        ...
        cm."total_orders",
        DATEDIFF('day', cm."first_order_date", CURRENT_DATE()) as "days_since_first_order"
...
WHERE "cohort_month" >= DATE_TRUNC('month', DATEADD(MONTH, -12, CURRENT_DATE()))
...
SELECT 
    "price_tier",
    COUNT(*) as "product_count",
    CONCAT('$', TO_CHAR(AVG("current_price"), 'FM999,999,999,990.00')) as "avg_price",
    SUM("total_quantity_sold") as "total_units_sold",
...
SELECT 
    ...
    CONCAT('$', TO_CHAR(AVG("customer_lifetime_value"), 'FM999,999,999,990.00')) as "avg_ltv",
    ROUND(AVG("total_orders"), 1) as "avg_orders",
    TO_CHAR(ROUND(AVG("category_diversity_score"), 2)) as "avg_diversity_score"
...
SELECT 'Revenue Decline Alert' as "alert_type",
       CONCAT('Recent 7-day revenue is down by $', 
              TO_CHAR("recent_revenue" - "previous_revenue", 'FM999,999,999,990.00')) as "message"
FROM (
    SELECT 
        SUM(CASE WHEN "date_key" >= DATEADD(DAY, -7, CURRENT_DATE()) THEN "gross_revenue" ELSE 0 END) as "recent_revenue",
        SUM(CASE WHEN "date_key" >= DATEADD(DAY, -14, CURRENT_DATE()) 
                  AND "date_key" < DATEADD(DAY, -7, CURRENT_DATE()) THEN "gross_revenue" ELSE 0 END) as "previous_revenue"
    FROM "sales_summary"
) revenue_comparison
